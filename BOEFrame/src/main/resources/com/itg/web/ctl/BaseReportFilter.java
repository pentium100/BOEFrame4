...
public class BaseReportFilter implements IReportFilter {
	...
	private String renderNorFilter(String userName, String prompt,
			String componentName, String xtype, String componentID,
	...
	private String getXmlData() {
		...
		try {
			...
			JCO.Function function = SAPConnect.createFunction(
					"ZRFCMAINDATA_COMM", destination);
			if (function == null)
				throw new RuntimeException(
						"ZRFCMAINDATA_COMM not found in SAP.");

			function.getImportParameterList().setValue("X", "ISALL");

			if (getTableIdField() != null) {
				function.getImportParameterList().setValue(
						"<TX><TABNAME>" + getTableName()
								+ "</TABNAME><FIELDNAME>" + getTableIdField()
								+ "</FIELDNAME></TX>", "PARAXML");
			} else {
				function.getImportParameterList().setValue(
						"<TX><TABNAME>" + getTableName()
								+ "</TABNAME><FIELDNAME>" + getIdField()
								+ "</FIELDNAME></TX>", "PARAXML");
			}

			System.out.println(" Echo: " + "<TX><TABNAME>" + getTableName()
					+ "</TABNAME><FIELDNAME>" + getIdField()
					+ "</FIELDNAME></TX>");
			...
			System.out.println(" Echo: "
					+ function.getExportParameterList().getString("OUTPUT"));
			System.out.println();
	...
	public String renderFilter(String userName, String prompt,
			String componentName, String componentID, String defaultValue,
			boolean isMulti, boolean allowBlank) {
		// TODO Auto-generated method stub

		if ((prompt.equals(""))
				&& (getLabel() != null && !getLabel().equals(""))) {
			prompt = getLabel();
		}
		...
		if (getMasterName().equals("datefield")
			...
			return renderNorFilter(userName, prompt, componentName,
					getMasterName(), componentID, defaultValue, allowBlank);
			...
			// JSONObject.fromObject(masterData.toString());;
			...
			if (getAuthObject() != null) {
				...
				for (int i = 0; i < a.size(); i++) {
					...
					authValue.add(a.getJSONObject(i).get(getIdField())
							.toString());
				}

				// System.out.println(jsonObject);

				List<String> authed = checkAuth(userName, getAuthObject(),
						authValue);
			...
			ds += "             id: '" + getIdField() + "'";
			ds += "                 }, ";
			ds += "         new Ext.data.Record.create([{name: 'id',type:'string', mapping:'"
					+ getIdField() + "'},";
			ds += "         {name:'text', mapping: '" + getTextField()
					+ "',type:'string'}]) ";
			ds += "        ) ";
			ds += "})";
			...
			if (isMulti) {
				...
				return this.genMultiSelect(userName, prompt, componentName,
						componentID, defaultValue, ds, allowBlank);
			} else {
				return this.genSingleSelect(userName, prompt, componentName,
						componentID, defaultValue, ds, allowBlank);
			}
	...
	private String genMultiSelect(String userName, String prompt,
			String componentName, String componentID, String defaultValue,
			String DS, boolean allowBlank) {
		// TODO Auto-generated method stub
	...
	private String genRemoteLoadMultiSelect(String userName, String prompt,
			String componentName, String componentID, String defaultValue,
			String DS) {
		// TODO Auto-generated method stub
		return null;
	...
	public String genSingleSelect(String userName, String prompt,
			String componentName, String componentID, String defaultValue,
			String DS, boolean allowBlank) {
		// TODO Auto-generated method stub
		String result = "  {columnWidth:.4, layout:'form', items:[";
